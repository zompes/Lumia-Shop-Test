import { Given, When, Then } from "@badeball/cypress-cucumber-preprocessor";

let searchedFor;

Given('that I have searched for the item wonderful {string}', (b) => {
  searchedFor = b;
  cy.get('#search').type(b);
});

When('I click the button to buy {string} a number of time', (quantity) => {
  for (let i = 1; i <= +quantity; i++) {
    cy.log("searchedFor", searchedFor);
    // find h2 that contains the product we have searched for
    cy.get('.product h2').contains(searchedFor)
      // find its product div
      .parents('.product')
      // find the buy button inside the product div
      .find('button').contains('Köp')
      // and click the button
      .click();
  }
});

Then('{string} {string} should be added to my shopping cart so that I can buy it', (quantity, b) => {
  cy.get('#cart li')
    .contains(b)
    // and also contains the correct quantity
    .contains(quantity + 'st')
    // we should 1 element
    .should('have.length', 1);
});